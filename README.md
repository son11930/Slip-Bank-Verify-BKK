...
บอท Discord สุดล้ำสำหรับ **ตรวจสอบสลิปโอนเงินอัตโนมัติ** รองรับทั้งสลิปธนาคาร (Bank Slip) และสลิป TrueMoney Wallet ทำงานรวดเร็วด้วย **EasySlip API** พร้อมระบบป้องกันสลิปซ้ำ (Duplicate Detection) และฟีเจอร์ตอบกลับข้อมูลบัญชีอัตโนมัติ

---

## 🌟 ฟีเจอร์หลัก (Features)

- 🔍 **อ่าน QR Code จากสลิปธนาคาร:** ดึงข้อมูล QR Payload เพื่อส่งไปตรวจสอบผ่าน EasySlip
- 🧡 **รองรับ TrueMoney Wallet:** ตรวจสอบสลิป TrueMoney ได้โดยตรงผ่านการอัปโหลดรูปภาพ
- 🛡️ **ป้องกันสลิปซ้ำ:** ตรวจสอบว่าสลิปนี้เคยถูกใช้งานไปแล้วหรือไม่ เพื่อป้องกันการทุจริต
...
- 👨‍💻 **ระบบ Manual Approval:** แอดมินสามารถกดยืนยัน (Approve) ยอดเงินและข้อมูลผู้โอน/ผู้รับได้หากสลิปมีความผิดปกติ 
...
3. **EasySlip API Key** (จำเป็นสำหรับการตรวจสอบสลิปของแท้)
...
4. **ตั้งค่า Environment Variables (`.env`):**
   ...
   - ใส่ข้อมูล Token และ API Key ของคุณ:
     ...
     EASYSLIP_API_KEY=ใส่_API_Key_ที่นี่
     ```
...
├── cogs/
│   ├── slip_verifier.py   # ระบบตรวจสอบสลิปและปุ่มแอดมิน
│   └── bank_info.py       # ระบบตอบกลับข้อมูลธนาคารและ /bank
├── utils/
│   ├── easyslip.py        # ตัวเชื่อมต่อ API กับ EasySlip
