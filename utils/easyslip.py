...
class EasySlipAPI:
    ...
    async def verify_bank_payload(self, payload: str, check_duplicate: bool = True) -> dict:
        """
        Verify Bank slip using QR Code Payload
        """
        if not self.api_key:
            raise ValueError("EASYSLIP_API_KEY is not set in environment variables.")
    ...
    async def verify_truewallet_image(self, image_bytes: bytes, check_duplicate: bool = True) -> dict:
        """
